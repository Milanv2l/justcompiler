...
def bootstrap_sandbox(target_path: Path, artifacts_path: Path, run_tests: bool, lang: str):
    ...
    # FIX 1: Complete set grafische & X11 dev libraries toegevoegd aan de basis image
...
"""
    ...
    # FIX 2: --auto-install toegevoegd aan de container runtime zodat AI-Rescue actief kan ingrijpen!
    run_cmd = docker_cmd + [
        "run",
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            CURRENT_STATUS = "Compilatie afgerond!"
            # FIX 3: Corrigeer valse succes-indicaties als de build logboeken pure mislukking aantonen
            if "Compilation failed completely" in result.stdout or "Compilatie volledig mislukt" in result.stdout:
                UI.error("Compilatie proces voltooid met fouten!")
            else:
